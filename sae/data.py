...
# def chunk_and_tokenize_chat_fast(
#     data: T,
...
#         chunk_size = min(tokenizer.model_max_length, max_seq_len)
#         # sep = tokenizer.eos_token or "<|endoftext|>"
#         sep = tokenizer.bos_token

#         instructions = x[instruction_key]
#         outputs = x[output_key]
        
#         # Measure time for applying chat template
...
#             # Find indices of eos_token_id
#             eos_indices = [i for i, token in enumerate(overflow) if token == tokenizer.eos_token_id]

#             # Initialize chunks with the first chunk from output
#             chunks = output["input_ids"]

#             # Start new chunks only at eos_token_id positions
#             start = 0
#             for end in eos_indices:
#                 if end - start >= chunk_size:
#                     start_str = tokenizer.decode(overflow[start:start+5])
#                     end_str = tokenizer.decode(overflow[start+chunk_size-5:start+chunk_size])
#                     print(f"Appending chunk start={repr(start_str)}... end={repr(end_str)}")
#                     chunks.append(overflow[start:start+chunk_size])
#                     start = end + 1
