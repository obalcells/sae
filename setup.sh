...
setup_vscode() {
    ...
    jq -r '.recommendations[]' ~/mats_hallucinations/.vscode/extensions.json | while read extension; do "$vscode" --install-extension "$extension"; done
    jq -r '.recommendations[]' ~/mats_hallucinations/.vscode/extensions.json | while read extension; do "$cursor" --install-extension "$extension"; done
}
